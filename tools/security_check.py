...
SECRET_PATTERNS = [
    ...
    (
        "secret_assignment",
        re.compile(r"(?i)\b(token|secret|password|passwd|api[_-]?key|ftp[_-]?password)\b\s*[:=]\s*['\"]?[^'\"\s]{8,}"),
    ),
]
...
ALLOWED_TRACKED_FILES = {"config.example.json", "ftp-sync.example.json", "ksef2jpk/config.json"}


def run_git(args: list[str]) -> list[str]:
    result = subprocess.run(
        ["git", *args], text=True, encoding="utf-8", errors="replace", capture_output=True, check=True
    )
...
def is_allowed_fixture(path: str) -> bool:
    path = normalize(path)
    return path in ALLOWED_TRACKED_FILES or any(path.startswith(prefix) for prefix in ALLOWED_TRACKED_PREFIXES)
...
def redact(value: str) -> str:
    ...
    value = re.sub(r"(?i)(token|secret|password|passwd|api[_-]?key)(\s*[:=]\s*)[^\s\"']+", r"\1\2***", value)
